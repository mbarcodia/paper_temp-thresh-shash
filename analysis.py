...
def plot_raw_temp_exceedance_percent_change_window(
    storyline_matches,
    d_test,
    d_obs,
    config,
    ...
    for i, match in enumerate(storyline_matches):
        ...
        # Step 5: Apply mask correctly
        exceedance_diff_masked = exceedance_diff.where(land_mask_xr)

        # ----- CNA Region Analysis -----

        # 1. Create 2D lat/lon grid
        lon2d, lat2d = np.meshgrid(count_before.lon.values, count_before.lat.values)

        # 2. Get CNA region object
        cna_region = regionmask.defined_regions.ar6.land[("CNA",)]

        # 3. Compute CNA mask (True = inside CNA)
        cna_mask = cna_region.mask(lon2d, lat2d)
        cna_mask = ~np.isnan(cna_mask)

        # 4. Convert to xarray aligned with your data
        cna_mask_xr = xr.DataArray(
            cna_mask,
            dims=["lat", "lon"],
            coords={"lat": count_before.lat, "lon": count_before.lon},
        )

        # --- CNA mask already computed above as cna_mask_xr ---
        # (expects cna_mask_xr to be an xr.DataArray aligned with count_before/count_after)

        # Ensure exceedance_diff exists (count_after - count_before)
        # make a version masked to CNA region
        exceedance_diff_cna = exceedance_diff.where(cna_mask_xr)

        # Boolean: True where the number of danger months increased after crossing
        increased_mask = exceedance_diff_cna > 0

        # Count number of CNA grid cells that had any increase
        num_cells_increased = int(increased_mask.sum().item())

        # Total CNA grid cells (land cells inside CNA on your grid)
        total_cna = int(cna_mask_xr.sum().item())

        # Percent of CNA grid cells that saw an increase
        percent_cells_increased = (
            (num_cells_increased / total_cna * 100.0) if total_cna > 0 else np.nan
        )

        # (Optional) You can also compute how many decreased or stayed same:
        num_cells_decreased = int((exceedance_diff_cna < 0).sum().item())
        num_cells_same = int((exceedance_diff_cna == 0).sum().item())

        # Print results
        print(f"\n--- CNA Increase Metrics for {ssp} - {model} member {member} ---")
        print(f"Crossing year: {year_cross}")
        print(f"Total CNA grid cells:                     {total_cna}")
        print(f"CNA grid cells with INCREASE (>0 months):  {num_cells_increased}")
        print(f"CNA grid cells with DECREASE (<0 months):  {num_cells_decreased}")
        print(f"CNA grid cells with NO CHANGE (==0):      {num_cells_same}")
        print(
            f"Percent of CNA that increased:             {percent_cells_increased:.2f}%"
        )
        print("--------------------------------------------------------------")
        ...
        # ---- Add percent increase annotation in top-right corner ----
        ax.text(
            0.987,
            0.98,
            f"+{percent_cells_increased:.0f}%",
            transform=ax.transAxes,
            ha="right",
            va="top",
            fontsize=14,
    ...
    if save == True:
        plt.savefig(
            f"{config['figure_dir']}analysis/{config['expname']}_slines_perc-CNA_danger_count_{temp_target}_{num_years}yravg_seed{seed}.png",
            dpi=300,
        )
...
def plot_avg_temp_after_threshold_by_ssp(
    ...
    for i, (ssp, entries) in enumerate(ssp_groups.items()):
        ...
        # 1. Subset the anomaly if needed
        if lat_range and lon_range:
        ...
        lon2d, lat2d = np.meshgrid(
            ssp_avg_anomaly.lon.values, ssp_avg_anomaly.lat.values
        )
        ...
        cna_mask_xr = xr.DataArray(
            cna_mask,
            dims=["lat", "lon"],
            coords={"lat": ssp_avg_anomaly.lat, "lon": ssp_avg_anomaly.lon},
        )

        # 1D cosine weights along latitude
        cos_weights = xr.DataArray(
            np.cos(np.deg2rad(ssp_avg_anomaly.lat.values)),
            dims=["lat"],
            coords={"lat": ssp_avg_anomaly.lat},
            name="cos_lat_weights",
        )

        # Apply CNA mask to anomalies
        ssp_masked = ssp_avg_anomaly.where(cna_mask_xr)  # your CNA mask as xarray

        # Weighted mean over lat/lon
        cna_weighted_mean = ssp_masked.weighted(cos_weights).mean(("lat", "lon")).item()

        # Simple min/max (unweighted)
        cna_min = ssp_masked.min(("lat", "lon")).item()
        cna_max = ssp_masked.max(("lat", "lon")).item()
        cna_count = int(cna_mask_xr.sum().item())  # number of grid cells in CNA

        print(
            f"{ssp} CNA weighted mean: {cna_weighted_mean:.2f}, min: {cna_min:.2f}, max: {cna_max:.2f}, cells: {cna_count}"
        )
        panel_labels = ["a)", "b)", "c)", "d)"]
        ...
        # ---------------------------------------------
        # Add stats to panel title
        # ---------------------------------------------
        # Set title with SSP name bolded and stats below
        ax.set_title(
            f"$\\bf{{{ssp}}}$\n"
            f"CNA: {cna_weighted_mean:.2f}°C (min={cna_min:.2f}, max={cna_max:.2f})",
            fontsize=12,
        )
        ...
        # Add (n=xx) in the top-right corner
        ax.text(
            0.98,
            0.98,
            f"n={len(anomaly_list)}",
            transform=ax.transAxes,
            ha="right",
            va="top",
            fontsize=12,
