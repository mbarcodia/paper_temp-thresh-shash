...
class Shash:
    ...
    * There is a typographical error in the presentation of the probability
    ...
    def mode(self, max_iter=100, tol=1e-8):
        ...
        Returns
        -------
        x_mode : float (batch size x 1) Tensor.
            The computed mode of the distribution.

        Notes
        -----
        * The mode is found by solving f'(x) = 0, where f(x) is the probability density function.
        * Uses Newton's method for root-finding.
        * Stops when the step size is smaller than the tolerance `tol` or max_iter is reached.
        """

        # Initial guess: Use the mean as a reasonable starting point
        x_mode = self.mu.clone().detach().requires_grad_(True)
        # x_mode = self.mu
        # x_mode = self.mu.clone().requires_grad_(True)
        # self.mu = torch.nn.Parameter(self.mu)  # Ensure it's a learnable parameter

        for _ in range(max_iter):
            # Compute first derivative f'(x) using autograd
            pdf_val = self.prob(x_mode)
            grad_1st = grad(pdf_val.sum(), x_mode, create_graph=True)[0]

            # Compute second derivative f''(x) using autograd
            grad_2nd = grad(grad_1st.sum(), x_mode, create_graph=False)[0]

            # Ensure the denominator is not too small to avoid division instability
            grad_2nd = torch.where(grad_2nd.abs() < tol, torch.tensor(tol), grad_2nd)

            # Newton-Raphson update: x_new = x - f'(x) / f''(x)
            step = grad_1st / grad_2nd
            x_mode = x_mode - step

            # Convergence check: Stop if step size is below tolerance
